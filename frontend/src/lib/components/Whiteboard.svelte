<script lang="ts">
	...
	function getStagePoint(): { x: number; y: number } {
	...
	function handlePointerMove(_e: Konva.KonvaEventObject<PointerEvent>) {
		if (!doc || !stage) return
		const pt = getStagePoint()
	...
	function handlePointerUp(_e: Konva.KonvaEventObject<PointerEvent>) {
