<script lang="ts">
    ...
    function ensureChatImageUrl(image: ChatImage): string {
        ...
        const objectUrl = URL.createObjectURL(
            base64ToBlob(image.data, image.mime),
        );
    ...
    let displayMessages = $derived(
        $activeChatTab === "regional" && $currentZoneId
            ? $regionalMessages
            : $chatMessages,
    );
    ...
    {#each displayMessages as message, i (i)}
        {#if message.isSystem}
            <div class="chat-entry chat-system">
                ...
                ><span class="chat-system-text"
                    >{#each parseTextWithUrls(message.text ?? "") as segment}{#if segment.type === "url"}<a
                                href={segment.value}
                                onclick={(e) =>
                                    handleLinkClick(e, segment.value)}
                                >{segment.value}</a
        ...
        {:else}
            <div class="chat-entry">
                ...
                ><span
                    class="chat-name"
                    style:color={message.nicknameColor ?? undefined}
...
<AlertDialog.Root bind:open={openLinkDialog}>
    <AlertDialog.Portal>
        ...
        <AlertDialog.Content class="link-dialog-content">
            <AlertDialog.Title class="link-dialog-title"
                >외부 사이트로 이동</AlertDialog.Title
            >
