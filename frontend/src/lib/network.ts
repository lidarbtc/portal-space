...
class NetworkClient {
	...
	connect(
		...
		onSnapshot?: (msg: OutgoingMessage) => void,
		position?: { x: number; y: number; reconnect?: boolean },
	...
	sendRegionalChatAction(
		objectId: string,
		settings: { name: string; radius: number; retainHistory: boolean },
	): void {
