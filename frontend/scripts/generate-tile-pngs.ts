...
function fillRect(png: PNG, sx: number, sy: number, w: number, h: number, hex: string) {
...
function strokeRect(png: PNG, sx: number, sy: number, w: number, h: number, hex: string) {
	const [r, g, b] = hexToRGB(hex)
	for (let x = sx; x < sx + w; x++) {
...
function fillCircle(png: PNG, cx: number, cy: number, radius: number, hex: string) {
	const [r, g, b] = hexToRGB(hex)
