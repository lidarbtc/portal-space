...
if (!portalCheck) {
	console.error(
		"Error: 'portal' CLI not found. Install from https://github.com/gosuda/portal-tunnel",
	)
	process.exit(1)
}
...
if (pids) {
	console.log(
		`[deploy] Killing existing processes on port 3000 (PIDs: ${pids.replace(/\n/g, ', ')})...`,
	)
...
const portal = Bun.spawn(
	[
		...
		'--description',
		'Portal Space — 2D pixel co-coding space',
		'--tags',
		'collab,portal-space',
		'--owner',
		'Portal Space',
		'--thumbnail',
		'https://space.portal.1ncursio.dev/assets/og-image.jpg',
		'--identity-path',
		'identity.json',
	],
	{
